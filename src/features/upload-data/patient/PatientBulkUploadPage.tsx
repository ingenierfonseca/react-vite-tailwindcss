import PageComponent from "../../../components/commons/PageComponent";
import StepComponent from "../components/StepComponent";
import { ArrowRight, Check, Download, File, Grid, Info, TriangleAlert, Upload, X } from "lucide-react";
import type { Header } from "../../../models/header.type";
import { useImportPatientHook } from "./useImportPatient.hook";
import { ASSETS_URLS } from "../../../config/constants";
import ErrorCard from "../components/ErrorCard";
import { getStatusBadge } from "../components/StatusBadge";

const headers: Header[] = [
  {
    header: 'Nombre',
    className: 'flex-1'
  },
  {
    header: 'Apellido',
    className: 'flex-1'
  },
  {
    header: 'Edad',
    className: 'flex-1'
  },
  {
    header: 'Telefono',
    className: 'flex-1'
  },
  {
    header: 'Email',
    className: 'flex-1'
  },
  {
    header: 'Estado',
    className: 'flex-1'
  },
  {
    header: 'Error',
    className: 'flex-1'
  }
]
export default function PatientBulkUploadPage() {
  const {
    stepActive,
    fileInputRef,
    previewData,
    errorResume,
    handleUploadClick,
    handleFileUpload,
    validateData,
    importData
  } = useImportPatientHook()
  let status: 'valido' | 'invalido' | 'duplicado' = 'valido';

  return (
    <PageComponent
      title="Importar Pacientes"
      description="Carga multiples pacientes usando un archivo Excel o CSV"
      textButton="Guia de Importacion"
      onclick={() => {
        alert("Nuevo")
      }}
      secondaryButton={<SecondaryButton />}>
      <div className="flex mt-5 px-6 py-3 border dark:border-slate-700 dark:bg-slate-800/50">
        <StepComponent stepActive={stepActive} stepNumber={1} title="Subir archivo" subTitle="Selecciona tu archivo" />
        <StepComponent stepActive={stepActive} stepNumber={2} title="Vista previa" subTitle="Revisa los datos" />
        <StepComponent stepActive={stepActive} stepNumber={3} title="Configurar e Importar" subTitle="Define opciones e importa" />
      </div>

      <div className="mt-4 flex flex-col md:flex-row gap-3">
        <div className="w-full md:max-w-100 shrink-0 p-6 border h-fit dark:border-slate-700 dark:bg-slate-800/50">
          <p className="dark:text-slate-200 font-bold">1. Subir archivo</p>
          <p className="text-xs dark:text-slate-400">Selecciona o arrastra el archivo con los datos de los pacientes</p>
          <div onClick={handleUploadClick}
            className="flex flex-col items-center mt-2 p-6 dark:bg-slate-900 rounded-lg border border-dashed cursor-pointer">
            <Upload className="text-primary" size={50} />
            <input
              type="file"
              ref={fileInputRef}
              onChange={handleFileUpload}
              className="hidden"
              accept=".xlsx, .csv"
            />
            <p className="mt-6 font-bold text-slate-200">Arrastra tu archivo aqui</p>
            <p className="text-xs dark:text-slate-400">o haz click para seleccionar</p>
            <p className="mt-6 text-primary">Formatos permitidos: .xlsx, .csv</p>
          </div>

          <div className="flex gap-3 p-2 mt-3 rounded-lg dark:bg-slate-900">
            <div className="flex-1">
              <div className="flex gap-2 items-center dark:text-slate-400">
                <File /><span className="text-[12px]">Tamaño máximo</span>
              </div>
              <p className="text-xs dark:text-slate-400">10 MB</p>
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 dark:text-slate-400">
                <Grid /><span className="text-[13px]">Formato</span>
              </div>
              <p className="text-[12px] dark:text-slate-400">Excel(.xlsx) o CSV</p>
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 dark:text-slate-400">
                <Info /><span className="text-[13px]">Codificación</span>
              </div>
              <p className="text-[12px] dark:text-slate-400">UTF-8 recomendado</p>
            </div>
          </div>
        </div>

        {stepActive > 1 && <div className="flex-2 flex flex-col gap-3">
          <div className="p-3 border dark:border-slate-700 dark:bg-slate-800/50">
            <div className="flex">
              <div>
                <p className="dark:text-slate-200 font-bold">2. Vista del archivo</p>
                <p className="text-xs dark:text-slate-400">Revisar los datos antes de importar</p>
              </div>
              <div className="flex items-center gap-2 ml-auto">
                <span className="text-sm dark:text-slate-400">Pacientes_importacion.xlsx</span>
                <div className="h-fit rounded-full bg-green-400"><Check size={14} /></div>
              </div>
            </div>

            <div className="flex mt-3 px-2 py-1.5 dark:bg-slate-800/50 border dark:border-slate-700">
              {headers.map((item) => (
                <div key={item.header} className={`${item.className} text-sm dark:text-slate-300`}>{item.header}</div>
              ))}
            </div>
            {previewData.map((patient, index) => (
              <div key={index} className="flex px-2 py-1.5 dark:bg-slate-900 dark:text-slate-200 border dark:border-slate-700">
                <div className="flex-1">{patient.Nombre}</div>
                <div className="flex-1">{patient.Apellido}</div>
                <div className="flex-1">{patient.Edad}</div>
                <div className="flex-1">{patient.Telefono}</div>
                <div className="flex-1">{patient.Email}</div>
                <div className="flex-1">{status.includes(patient.Estado) ? getStatusBadge(patient.Estado as "valido" | "invalido" | "duplicado") : <div></div>}</div>
                <div className="flex-1">{patient.Error??'---'}</div>
              </div>
            ))}

            {stepActive === 2 &&<button onClick={validateData}
              className="ml-auto mt-3 bg-primary text-white rounded-lg p-2.5 cursor-pointer">Validar datos
            </button>}
          </div>
        </div>}
      </div>

      {stepActive > 2 &&<div className="mt-3 p-3 border dark:border-slate-700 dark:bg-slate-800/50">
        <p className="dark:text-slate-200 font-bold">3. Importacion</p>
        <p className="dark:text-slate-200 font-bold">Resumen de validacion</p>
        <p className="text-xs dark:text-slate-400">Total de registros en el archivo: {previewData.length} </p>

        <div className="flex gap-3 mt-3">
          <ErrorCard value={errorResume!.ok}
            title="Validos"
            description="Listos para importar"
            bgColor="green-400"
            textColor="text-green-400"
          >
            <Check size={35} className="p-0.5 text-green-400 rounded-full border border-green-400" />
          </ErrorCard>

          <ErrorCard value={errorResume!.error}
            title="Con errores"
            description="Requieren correccion"
            bgColor="red-400"
            textColor="text-red-400"
          >
            <X size={35} className="p-0.5 text-red-400 rounded-full border border-red-400" />
          </ErrorCard>

          <ErrorCard value={errorResume!.duplicated}
            title="Duplicados"
            description="Existen varios"
            bgColor="yellow-400"
            textColor="text-yellow-400"
          >
            <TriangleAlert size={35} className="text-yellow-400" />
          </ErrorCard>
        </div>
        <div className="flex mt-3">
          {errorResume.ok > 0 && <button className="flex gap-2 ml-auto bg-primary rounded-lg text-white p-2.5"
            onClick={importData}>
            Finalizar Importación
            <ArrowRight />
          </button>}

          {errorResume.ok === 0 && <div>
            <p className="text-red-400">No hay datos validos para procesar</p>
          </div>}
        </div>
      </div>}
    </PageComponent>
  );
}

function SecondaryButton () {
  return (
    <a className="flex gap-2 items-center rounded-lg border px-3 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
      href={ASSETS_URLS.fileImportPatient}
      download="planpacientes_importaciontilla.xlsx">
      <Download />Descargar plantilla
    </a>
  )
}
//<a href="/plantillas/mi-plantilla.xlsx" download="plantilla.xlsx">