...
import { useImportClinicalHistoryHook } from "./useImportClinicalHistory.hook";
...
const headers: Header[] = [
  { header: 'Fecha',              className: 'w-30' },
  { header: 'Motivo Consulta',    className: 'flex-1' },
  { header: 'Notas Clínicas',     className: 'flex-1' },
  { header: 'Doctor',             className: 'flex-1' },
  { header: 'Especialidad',       className: 'flex-1' },
  { header: 'Estado',             className: 'w-28' },
  { header: 'Error',              className: 'flex-1' },
];

export default function ClinicalHistoryBulkUploadPage() {
  const { patientId } = useParams();
  const {
    stepActive, fileName, previewData, errorResume,
    currentItems, currentPage, totalPages, isImporting,
    importResult, customerName, handleChangePage,
    handleFileUpload, validateData, importData,
    resetData, clearImportResult,
  } = useImportClinicalHistoryHook(Number(patientId)!);
  ...
  return (
    <PageComponent
      ...
      description={`Carga el historial clínico del paciente desde un archivo Excel${customerName ? ` — ${customerName}` : ''}`}
    ...
    >
      ...
      <div className="mt-4 flex flex-col md:flex-row gap-3">
        ...
        <div className="w-full md:max-w-100 shrink-0 p-6 border border-slate-300 h-fit dark:border-slate-700 dark:bg-slate-800/50">
          ...
          <p className="text-xs dark:text-slate-400">Selecciona o arrastra el archivo con el historial clínico del paciente</p>
          ...
          <div className="flex gap-3 p-2 mt-3 rounded-lg dark:bg-slate-900">
            <div className="flex-1">
              <div className="flex gap-1 items-center dark:text-slate-400"><File size={20} /><span className="text-[12px]">Tamaño máximo</span></div>
              <p className="ml-6 text-xs dark:text-slate-400">2 MB</p>
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-1 dark:text-slate-400"><Grid size={20} /><span className="text-[13px]">Formato</span></div>
              <p className="text-[12px] dark:text-slate-400">Excel(.xlsx) o CSV</p>
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-1 dark:text-slate-400"><Info size={20} /><span className="text-[13px]">Codificación</span></div>
          ...
          <div className="p-3 border border-slate-300 dark:border-slate-700 dark:bg-slate-800/50">
            ...
            {currentItems.map((row, index) => (
              <div key={index} className="flex px-2 py-1.5 dark:bg-slate-900 text-sm text-black dark:text-slate-200 border border-slate-100 dark:border-slate-700">
                <div className="w-30">{row.Fecha}</div>
                <div className="flex-1 font-semibold truncate">{row.MotivoConsulta}</div>
                <div className="flex-1 truncate">{row.NotasClinicas || '---'}</div>
                <div className="flex-1 truncate">{row.Doctor || '---'}</div>
                <div className="flex-1 truncate">{row.Especialidad || '---'}</div>
                <div className="w-28">{row.Estado !== undefined ? getStatusBadge(row.Estado as "valido" | "invalido" | "duplicado") : <div></div>}</div>
                <div className="flex-1 truncate min-w-0">{row.Error ?? '---'}</div>
              </div>
            ))}

            <PaginatedFoot sizeData={previewData.length} totalPages={totalPages} currentPage={currentPage} onPageChange={(page) => handleChangePage(page)} />

            {stepActive === 2 && <button onClick={validateData} className="ml-auto mt-3 bg-primary text-white rounded-lg p-2.5 cursor-pointer">Validar datos</button>}
          </div>
        </div>}
      </div>

      {/* Step 3 - Validation summary & import */}
      {stepActive > 2 && <div className="mt-3 p-3 border border-slate-300 dark:border-slate-700 dark:bg-slate-800/50">
        ...
        <div className="flex gap-3 mt-3">
          <ErrorCard value={errorResume!.ok}     status="valido"    title="Validos"       description="Listos para importar" />
          <ErrorCard value={errorResume!.error}  status="invalido"  title="Con errores"   description="Requieren correccion" />
          <ErrorCard value={errorResume!.duplicated} status="duplicado" title="Duplicados" description="Ya existen en el historial" />
        </div>

        <div className="flex mt-3">
          {errorResume.ok > 0 && <button className="flex gap-2 ml-auto bg-primary rounded-lg text-white p-2.5 cursor-pointer" onClick={importData}>
            Finalizar Importación <ArrowRight />
          </button>}
          {errorResume.ok === 0 && <p className="text-red-500 font-semibold">No hay datos validos para procesar</p>}
        </div>
      </div>}
      ...
      {isImporting && createPortal(
        <div className="fixed inset-0 z-100 flex flex-col items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="flex flex-col items-center gap-3 p-8 rounded-2xl bg-white dark:bg-slate-800 shadow-2xl">
            ...
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Importando historial clínico...</p>
          </div>
        </div>,
        document.body
      )}
...
interface SecondaryButton { className: string }
function SecondaryButton({className}: SecondaryButton) {
  return (
    <a className={`${className} hidden md:flex gap-2 min-w-0 py-2.5 items-center rounded-lg border px-3 dark:border-slate-700 dark:text-slate-200 hover:bg-slate-300 dark:hover:bg-slate-800`}
      href={ASSETS_URLS.fileImportClinicalHistory}
      download="historial_clinico_importacion.xlsx">
