    ...
    return (
        <div className="bg-white">
                ...
                <div className="flex mt-2">
                    <div className="flex-1 flex-col">
                        ...
                        <button onClick={handleAddItem}
                            disabled={loading}
                            className={`mt-2 flex disabled:opacity-50 ${cn(theme.button.base)} bg-slate-200`}>
                            <Plus />
                            Agregar Linea
                        </button>
                    </div>
                    <div className="w-70 h-fit border border-slate-200 bg-slate-100 ml-2 text-sm">
                        <div className="border-b-2 border-slate-200 flex justify-between p-2"><span>SubTotal</span><span>{invoice !== null ? calculateTotal() : "0.00"}</span></div>
                        <div className="border-b-2 border-slate-200 flex justify-between p-2"><span>IVA (0%)</span><span>{invoice !== null ? formatNumber(invoice?.taxTotal) : "0.00"}</span></div>
            ...
            <div className="flex justify-end px-4 py-3 gap-1">
                <button className={`disabled:opacity-50 ${cn(theme.button.base)}`}
                    disabled={loading}
                    onClick={() => navigate(`/invoice`)}>Cancelar</button>
                <button className={`flex items-center gap-2 disabled:opacity-50 ${cn(theme.button.base)} ${cn(theme.button.variant.primary)}`} onClick={saveInvoice} disabled={loading}>
