...
const headers: Header[] = [
    ...
    { header: "Tipo", className: "flex-2" },
    { header: "Capacidad", className: "flex-1" },
    { header: "Estado", className: "flex-1" },
];

const RESOURCE = PermissionResource.Resources;
...
export default function ResourceListPage() {
    ...
    return (
        <PageComponent
            title="Recursos"
            description="Administra los recursos físicos de la clínica"
            textButton="Agregar Recurso"
        ...
        >
                ...
                <div className="divide-y divide-slate-200 dark:divide-slate-600">
                    {data?.data?.map((item, index) => (
                        <div key={item.id}
                            ...
                            <span className="flex-2 dark:text-slate-200 font-medium">{item.name}</span>
                            <span className="flex-2 dark:text-slate-400">{item.resourceTypeId}</span>
                            <span className="flex-1 dark:text-slate-200">{item.capacity}</span>
                            <span className="flex-1 dark:text-slate-200">{item.isActive ? "Activo" : "Inactivo"}</span>
            ...
            <div className={`fixed top-0 right-0 h-full w-full md:w-7/12 bg-white dark:bg-slate-800 shadow-2xl z-50 
                ...
                {isOpenCreateOrEdit && <ResourceForm setIsOpen={openCreate} itemParam={item} reload={load} />}
            </div>
        </PageComponent>
    );
}
